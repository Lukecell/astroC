...
bool ever_visible_altitude(struct observatory obs, struct celestial_object obj, double min_alt_deg) {
...
vec_double find_valid_altitude_intervals(struct observatory obs, struct celestial_object obj, double min_alt_deg) {
	double min_alt_rad = deg_to_rad(min_alt_deg);
	...
	for (double hour_angle_rad = PI * -1; hour_angle_rad <= PI; hour_angle_rad += PRECISION_CONSTANT) {
		...
		double alt_at_curr_time = find_altitude_rad(sin_const, cos_const, hour_angle_rad + lon_rad);
...
int main()
{
	...
	ECS polaris_coord = {.dec = 80.5, .ra = 10};
	char *ptr;
	struct celestial_object polaris = {.coord = polaris_coord, .name=0};
	
	vec_double intervals = find_valid_altitude_intervals(south_pole, polaris, (double) 89.5);
